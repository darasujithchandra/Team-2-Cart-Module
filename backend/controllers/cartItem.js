...
const patchCartItems = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    console.log("set")
    console.log("id", req.body.bookid, req.params.email, req.body.quantity)
    res.status(201).json({ success: true })
  } catch (error) {
    console.log("caught")
  }
  ...
  // const query = { "email": req.params.email, "books.bookid": req.params.id };
  // const updateDocument = {
  //   $set: { "books.$.quantity": req.body.value }
  // };
  // const result = await CartItem.updateOne(query, updateDocument);
  // const p = await CartItem.find({ "email": req.params.email, "books.bookid": req.params.id })
  // console.log("p", p)
  // const query = { email: req.params.email };
  // const updateDocument = {
  //   $set: { "books.$[orderItem].quantity": req.body.quantity }
  // };
  // const options = {
  //   new: true,
  //   runValidators: true,
  //   arrayFilters: [{
  //     "orderItem": mongoose.Types.ObjectId(req.params.id),
  //   }]
  // };
  // const result = await CartItem.updateMany(query, updateDocument, options);
